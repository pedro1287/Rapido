...
def processUploadFiles(filename,filesize,files,update,bot,message,thread=None,jdb=None):
    try:
        bot.editMessageText(message,'☁️Preparandose Para Subir⚡️...')
        evidence = None
        fileid = None
        ...
        if cloudtype == 'moodle':
            ...
            if loged:
                ...
                bot.editMessageText(message,'🌐Ups! Hubo un problema al intentar conectarse con la nube✖️')
...
def processFile(update,bot,message,file,thread=None,jdb=None):
    ...
    if client:
        ...
        try:
            ...
            bot.sendMessage(message.chat.id,'✖️NO SE PUDO MOVER A CALENDARIO✖️')
    else:
        bot.editMessageText(message,'🌐Ups! Hubo un problema al intentar conectarse con la nube✖️')
    ...
    try:
        ...
        if '/add' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    bot.sendMessage(update.message.chat.id,f'🚫Error en el comando /add usuario')
            else:
                bot.sendMessage(update.message.chat.id,'🚫Lo sentimos usted no tiene permisos de administrador🚫')
            return
        if '/admin' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
                try:
                    ...
                    bot.sendMessage(update.message.chat.id,f'🚫Error en el comando /admin usuario🚫')
            else:
                bot.sendMessage(update.message.chat.id,'🚫Lo sentimos usted no tiene permisos de administrador🚫')
            return
        ...
        if '/prueba' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    bot.sendMessage(update.message.chat.id,f'🚫Error en el comando /preview usuario🚫')
            else:
                bot.sendMessage(update.message.chat.id,'🚫Lo sentimos usted no tiene permisos de administrador🚫')
            return 
        if '/ban' in msgText:
            ...
            if isadmin:
                try:
                    user = str(msgText).split(' ')[1]
                    if user == username:
                        bot.sendMessage(update.message.chat.id,'🚫No puede banearse a si mismo🚫')
                        return
                    jdb.remove(user)
                    jdb.save()
                    ...
                    bot.sendMessage(update.message.chat.id,'🚫Error en el comando /ban usuario🚫')
            else:
                bot.sendMessage(update.message.chat.id,'🚫Lo sentimos usted no tiene permisos de administrador🚫')
            return
        ...
        if '/zips' in msgText:
            ...
            if getUser:
                try:
                   ...
                   bot.sendMessage(update.message.chat.id,'🚫Error en el comando /zips tamaño de zips🚫')    
                return
        if '/gen' in msgText:
            pass444
        if '/acc' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'🚫Error en el comando /acc usuario,contraseña🚫')
            return
        ...
        if '/host' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'🚫Error en el comando /host url de la nube🚫')
            return
        if '/repo' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
                ...
                bot.sendMessage(update.message.chat.id,'🚫Error en el comando /repo ID de la moodle🚫')
            return
        if '/encript_on' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'🚫Error en el comando /encrypt_on estado de Encriptar🚫')
            return
        if '/encript_off' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'🚫Error en el comando /encript_off estado de Encriptar🚫')
            return
        if '/nube' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
        ...
        if '/up' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'🚫Error en el comando up tipo de subida (evidence,draft,blog,calendar)🚫')
            return
        ...
        if '/dir' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'🚫Error en el comando /dir carpeta destino🚫')
            return
        ...
        if '/start' in msgText:
            start_msg = ' 📶SpeedUpload 2.0📤\n'
            start_msg+= '࿇══━━━━━━✥◈✥━━━━━━══࿇\n'
            start_msg+= '💣Hola @' + str(username)+'\n'
            start_msg+= '✅Bienvenid@ al bot de descargas gratis SpeedUpload 2.0 en su versión 2.0 Plus⚜️!\n'
            start_msg+= '🖥Desarrollador: @TuguerX\n\n'
            start_msg+= '🆘Si necesita ayuda o información utilice:\n'
            start_msg+= '/ayuda -- /acerca\n'
            start_msg+= '😁Disfrute al máximo de su estancia en el bot🤩.\n'
            ...
            #    return
            bot.editMessageText(message,'🚫Error, No se pudo analizar correctamente🚫')
    except Exception as ex:
           print(str(ex))
  

def main():
    bot_token = os.environ.get('5488241099:AAGWZZ59WrcgsDP1e8CY04Pn8Gwna6Fh4b0')
